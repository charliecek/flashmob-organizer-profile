    ...
    <form class="lwa-form" action="<?php echo esc_attr( LoginWithAjax::$url_login ); ?>" method="post" style="display:none;">
        <div class="lwa-username">
          <?php $msg = "Prihlasovacie meno alebo e-mailová adresa"; ?>
            ...
            <input type="text" name="log" id="registration-form-only_lwa_user_login" class="input" value="<?php echo esc_attr( $msg ); ?>"
                   onfocus="if(this.value == '<?php echo esc_attr( $msg ); ?>'){this.value = '';}" onblur="if(this.value == ''){this.value = '<?php echo esc_attr( $msg ); ?>'}"/>
        </div>

        <div class="lwa-password">
          <?php $msg = "Heslo"; ?>
            <label for="registration-form-only_lwa_user_pass"><?php esc_html_e( $msg, 'login-with-ajax' ); ?></label>
            <input type="password" name="pwd" id="registration-form-only_lwa_user_pass" class="input" value="<?php echo esc_attr( $msg ); ?>"
                   onfocus="if(this.value == '<?php echo esc_attr( $msg ); ?>'){this.value = '';}" onblur="if(this.value == ''){this.value = '<?php echo esc_attr( $msg ); ?>'}"/>
        </div>
        ...
        <div class="lwa-submit-button">
            <input type="submit" name="wp-submit" id="registration-form-only_lwa_wp-submit" value="<?php echo "Prihlásiť sa"; ?>" tabindex="100"/>
