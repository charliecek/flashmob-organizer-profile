	...
	//Remember and register form AJAX
 	$('form.lwa-form, form.lwa-remember, div.lwa-register form').submit(function(event){
		...
		$.ajax({
			...
			error : function(){ console.warn(arguments); lwaAjax({}, statusElement); },
			dataType : 'json'
		});
		//trigger event
	});
...
// 					lwaOrg.replaceWith(lwaSub);
				...
				} else if (window.location.href.match(new RegExp(data.redirect.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&")+'$', 'gi'))) {
          window.location.reload();
        } else {
